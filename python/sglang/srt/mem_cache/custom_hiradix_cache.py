...
class CustomHiRadixCache(RadixCache):
    ...
    def _should_backup_to_host(self, node: TreeNode) -> bool:
        # Reserve host memory for longer reusable spans and skip short fragments.
        return len(node.key) >= self.min_backup_len
    ...
    def _inc_hit_count(self, node: TreeNode, chunked=False):
        ...
        if not node.backuped and self._should_backup_to_host(node):
        ...
        while num_evicted < num_tokens and len(eviction_heap):
            ...
            if not x.backuped:
                if self.cache_controller.write_policy == "write_back":
                    if self._should_backup_to_host(x):
                        # write to host if the node is not backuped

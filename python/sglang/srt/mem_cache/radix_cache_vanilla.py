...
class VanillaRadixCacheImpl(BasePrefixCache):
    ...
    def inc_lock_ref(self, node: TreeNode):
        if self.disable:
            logger.warning("[RADIX TRACE] inc_lock_ref skip reason=disabled")
            return 0
    ...
    def dec_lock_ref(self, node: TreeNode):
        if self.disable:
            logger.warning("[RADIX TRACE] dec_lock_ref skip reason=disabled")
            return 0
...
if __name__ == "__main__":
    tree = VanillaRadixCacheImpl.create_simulated()
